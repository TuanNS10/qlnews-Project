...
module.exports = {
  ...
  slide1: function () {
    ...
    where n.statusid = 1 and n.time <NOW() AND n.ispremium=0   order by view,rand() limit 1,1`);
  },
  slide2: function () {
    return db.load(`select n.*,t.id as t_id,  t.name as t_name from ${TBL_NEWS} n join ${TBL_TOPIC} t on n.topicid=t.id
    where  n.statusid = 1 and n.time <NOW() AND n.ispremium=0   order by view,rand() limit 2,1`);
  },
  slide3: function () {
    return db.load(`select n.*,t.id as t_id,  t.name as t_name from ${TBL_NEWS} n join ${TBL_TOPIC} t on n.topicid=t.id
    where n.statusid = 1 and n.time <NOW() AND n.ispremium=0 order by view, rand() limit 3,1`);
  },
  top10: function () {
    return db.load(`select n.*,t.id as t_id, t.name as t_name  from ${TBL_NEWSPAPER} n join ${TBL_TOPIC} t 
  on n.topicid=t.id WHERE  n.statusid = 1 and n.time <NOW() order by  time DESC, rand() limit 10`);
  },
  randomtop5: function () {
